...
SPEC="safeAI_Verify_5000_100_0.05_0.2.vcl"
NETWORK="safeNLP:../results/adversarial.onnx"
RESULT_FILE="hyperrectangle_results/safeAI_Verify_5000_100_0.05_0.2_results.txt"
...
do
    ...
    if echo "$OUTPUT" | grep -q "found a counterexample"; then
        ...
        echo "property$i: TIMED OUT" | tee -a "$RESULT_FILE"
    else
        echo "property$i: UNKNOWN RESULT" | tee -a "$RESULT_FILE"
    fi
